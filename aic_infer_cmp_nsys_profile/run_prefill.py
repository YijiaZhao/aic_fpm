...
def main():
    ...
    for i, req_info in enumerate(record["request_infos"]):
        ...
        if i < 3:
            print(
                f"  req[{i}] rid={rid[:16]}... prefix={prefix_len} extend={extend_len} full={prefix_len + extend_len}"
            )
    ...
    piecewise_max_tokens = total_extend_tokens
    ...
    if not is_glm5:
        ...
        print(f"[piecewise] non-GLM max seq len={piecewise_max_tokens}")
